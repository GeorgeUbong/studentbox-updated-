...
// 1. Move the assessment logic to a sub-component
function TestContent() {
  ...
  if (!testId) return (
    <div className="p-10 text-center">
      <p className="text-zinc-500 font-bold mb-4">Invalid Test ID</p>
      <button onClick={() => router.push('/Assessments')} className="text-[#1976D2] font-black underline">
  ...
  if (!assessment) return <div className="h-96 w-full rounded-3xl bg-zinc-100 animate-pulse" />;
    ...
    <>
      {isSubmitted ? (
        ...
        (() => {
          ...
          const isPassing = (score / totalQuestions) >= 0.5;

          return (
            <div className="bg-white p-12 rounded-[2rem] text-center border border-zinc-200 shadow-sm animate-in zoom-in-95 duration-500">
              <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${
                isPassing ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
              }`}>
                {isPassing ? <Trophy size={40} className="animate-bounce" /> : <AlertCircle size={40} />}
              </div>
        ...
        <div className="animate-in fade-in duration-500">
          <div className="mb-8">
            <h1 className="text-4xl font-black text-black mb-2">{assessment.title}</h1>
            <p className="text-zinc-500 font-bold">{currentStep + 1} of {totalQuestions} Questions</p>
          </div>

          <div className="bg-[#1976D2] rounded-[2rem] p-8 md:p-12 shadow-2xl relative">
                ...
                return (
                  <button
                    ...
                    className={`group p-4 text-left rounded-2xl border-2 transition-all flex items-center gap-4 ${
                      isSelected ? 'bg-white text-[#1976D2] border-white' : 'bg-transparent text-white border-white/40 hover:border-white'
                    }`}
                  >
                    ...
                    <span className="text-lg font-bold flex-1">{option.text}</span>
          ...
          <div className="mt-8 text-center">
            ...
            >
              <ChevronLeft size={18} /> Quit Assessment
            </button>
          </div>
        </div>
      )}
